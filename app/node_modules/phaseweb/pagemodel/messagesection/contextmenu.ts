...
class ContextMenu {
    ...
    private registerCopyMessage(): void {
        $(ElementIdentifiers.ContextMenu.messageCopy).on('click', () => {
            const client = this._client;
            const message = (client.currentDiscussion ?? client.deepHistory ?? client.discussionSearch)?.messages.find(m => m.id === this._contextMenuMessageId);;
            if (message) {
                const text = message.content.asIs();
                const timestamp = message.timestamp;
                const ip = message.ip;
                const username = message.getDisplayName().asIs();
                const timeZoneOffset = new Date().getTimezoneOffset();
                const utcTimestamp = timestamp + timeZoneOffset * 60;
                const reformattedDate = Moment(utcTimestamp * 1000).format('ddd, MMM DD, YYYY HH:mm:ss | hh:mm:ssA UTC');

                const input = document.createElement("textarea");
                let copied = `> ${text}\n`;
                copied += `> - \`${username}\`(${ip}) @ <t:${timestamp}:f> / \`${reformattedDate}\`\n`;
                copied += `> https://phase.dark-gaming.com/#messageref_${this._contextMenuMessageId}\n`;
